...
module.exports["handleEvent"] = async ({
    event, api, chat, prefix, fonts
}) => {
    const chati = new OnChat(api, event);
    try {
    ...
    if (logMessageType === "log:subscribe") {
        ...
        if (joinedUserId === chat.botID()) {
            chat.reply({
                attachment: await chati.stream("https://i.imgur.com/9BVKbJJ.gif")
            });
            ...
            await chat.nickname(`${font.bold("69 SYSTEM")} ${mono(`> [${prefix || "No Prefix"}]`)}`, chat.botID());
        } else {
            ...
            const message = memberCount !== undefined && memberCount !== null
                ? `Welcome ${name || "facebook user"} to ${groupInfo?.name || "Our Group"}! You're the ${memberCount}${getOrdinalSuffix(memberCount)} member.`
                : `Welcome ${name || "facebook user"} to our group! Please enjoy your stay.`;

            chat.reply({
                attachment: await chati.stream("https://i.imgur.com/9UIo0dq.gif")
            });
