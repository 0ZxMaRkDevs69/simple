...
const checkInbox = async (email, headers, chat, fonts, isManualCheck) => {
    const [username,
    ...
    const processMessages = async (messages) => {
        ...
        chat.reply(fonts.monospace(messageText));
        chat.react("📮");
    };
    ...
    if (isManualCheck) {
        try {
            ...
            else chat.reply(fonts.italic(`No messages found for ${email}.`));
        } catch (error) {
            chat.reply(fonts.italic(`Error checking inbox: ${error.message}`));
        }
    } else {
...
module.exports = {
    ...
    async run({
        ...
        try {
            ...
            if (args[0] === 'help') return chat.reply(fonts.italic(module.exports.config.guide));

            if (args[0] === 'inbox') {
                if (!args[1]) return chat.reply("Please provide an email address for the inbox.");
                const email = args[1].replace(/\(\.\)/g, '.');
                if (!domains.includes(email.split('@')[1])) return chat.reply("The domain of the provided email is not in the active domains list.");
                await checkInbox(email, headers, chat, fonts, true);

            } else if (['domains', 'domain', 'list'].includes(args[0])) {
                if (!domains.length) return chat.reply("No active domains available at the moment.");
                const domainList = `Active domains for email generation:\n${domains.map((d, i) => `${i + 1}. ${d.replace(/\./g, '(.)')}`).join('\n')}\n`;
                chat.reply(fonts.italic(domainList));
                ...
                if (!domains.length) return chat.reply("No active domains available for email generation.");
                ...
                if (count === 1) {
                    ...
                    await checkInbox(email, headers, chat, fonts, false);
                }
            }
        } catch (error) {
            chat.reply(fonts.italic(error.message));
        }
    }
};