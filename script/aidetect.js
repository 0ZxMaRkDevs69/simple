...
module.exports["run"] = async ({ chat, event, args, fonts, global }) => {
    ...
    const getTextToAnalyze = () => {
        if (event.type === "message_reply") {
            return fonts.origin(event.messageReply.body);
        }
        return fonts.origin(args.join(' '));
    };

    const text = getTextToAnalyze().trim();
    if (!text) {
        return chat.reply(fonts.thin('Please provide the text to check or reply to a message containing the text.'));
    }

    const detecting = await chat.reply(fonts.monospace("Detecting AI Generated Text..."));
    ...
    if (result.status) {
        ...
        chat.reply(fonts.thin(`Detection result:
        ...
        chat.reply(fonts.monospace(`An error occurred while detecting the text: ${result.error}`));
    }
};
